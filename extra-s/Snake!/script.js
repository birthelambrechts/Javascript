...
function Board(containerId, rowsCount, colsCount) {
    ...
    this.render = function () {// aanmaken van het bord
        ...
        html += "</table><div class='preview'></div><div class='score'><p> Score: <span class='score-value'></span></p></div><button type='button'>Play!</button></div>";
        document.getElementById(containerId).innerHTML = html;
    };

    this.hidePreview = function () { // display at the end
...
function Snake(board, speed, head) {
    ...
    this.start = function () {
        ...
        function moveHead() { // hoofd
            ...
            if (fruit) {
                removeClass(nextHeadElement, "fruit"); // fruit komt bij dus ook langere slang
            }
        ...
        function moveCellsAfterHead($snake) { //vakjes volgen elkaar
            var newCells = [];
    ...
    this.grow = function () {
        ...
        this.tail = newTail;
        $snake.board.setScoreValue($snake.cells.length * 10);
    };
...
function addFruitToBoard(board, snake) { // fruitjes
    do {
...
function addKeyboardListener(snake) { //knopjes
...
function addSwipeListener(snake) { //mobile
...
function direction(keyCode) { //pijltjes
...
function Cell(row, col, direction) {
    ...
    this.prev = function () { //volgende fruitje
        switch (this.direction) {
            case "up":
                return new Cell(this.row + 1, this.col, this.direction);
            case "down":
